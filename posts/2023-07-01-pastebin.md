...
Reset Your Photos wallpapers:

```sh
killall WallpaperAgent
rm -rf ~/Library/Containers/com.apple.wallpaper.extension.image/Data/Library/Preferences/com.apple.wallpaper.extension.image.plist ~/Library/Application\ Support/com.apple.wallpaper/
```
...
```
# Long line in zsh history
perl -ne 'print if length($_) > 120' ~/.zsh_history
```
...
git config --global user.signingkey ~/.ssh/XXX_ed25519.pub
...
- `-a`: archive `-rlptgoD`
- `-v`: verbose
- `-z`: compress, use when sync via network
...
```sh
# Compress
# -c:v libx265 -crf 26 -preset fast: control quality
# -an: no audio
# -c:a aac -b:a 128k: audio codec and bitrate
ffmpeg -i input -vf "fps=30" -c:v libx265 -crf 26 -preset fast -an output.mp4

# Compress on macOS
# -c:v hevc_videotoolbox -b:v 2500k -tag:v hvc1: control quality
ffmpeg -i input -vf "fps=30" -c:v hevc_videotoolbox -b:v 2500k -tag:v hvc1 -an output.mp4

# Remove unwanted stream
# -map 0: include all streams
# -map -0:1: remove stream 1
ffmpeg -i src.mkv -map 0 -map -0:1 -c copy dst.mkv
```
...
vips jxlsave filename.jpg filename.jxl
vips jpegsave filename.png filename.jpg

# Batch convert (require fd)
fd "jpg|jpeg|png|webp|gif" --exec vips jxlsave {} {.}.jxl
fd "jpg|jpeg|png|webp|gif" --exec-batch rm

fd "png|webp|gif" --exec vips jpegsave {} {.}.jpg
...
```sh
# Show all time tags
exiftool -time:all -s Untitled.jpg

# Batch rename
exiftool "-FileName<FileModifyDate" "-FileName<ModifyDate" "-FileName<MediaCreateDate" "-FileName<CreateDate" "-FileName<DateTimeOriginal" -d "%Y%m%d_%H%M%S_%%.2c.%%e" .
```
