...
Don't use MySQL type timestamp, use int64 then pass the timestamp (prefer UnixMilli) to avoid date, time, location
complex conversion.
...
Default `DATETIME`, `TIMESTAMP` in MySQL is round to seconds.
...
Be careful, MySQL sorts the keys of JSON object.
